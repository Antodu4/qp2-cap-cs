...
subroutine mo_one_rdm_cs(coefs, N_st, sze, rdm)
  implicit none

  BEGIN_DOC
  ! Computes the one-electron density matrix in the MO basis using the
  ! c-inner product (biorthogonal, no complex conjugation), for the CS
  ! wave function with complex CI coefficients.
  !
  ! gamma_pq = sum_{ij} C_i * C_j * <i| a^+_p a_q |j>
  !
  ! Diagonal (degree-0) contributions come from occupied orbital lists;
  ! off-diagonal (degree-1) contributions from single excitations.
  !
  ! Input:
  !   coefs(sze, N_st)  -- complex CI coefficients
  !   N_st              -- number of electronic states
  !   sze               -- number of determinants
  ! Output:
  !   rdm(mo_num, mo_num, N_st) -- complex 1-RDM
  END_DOC
...
BEGIN_PROVIDER [ double precision, dm, (mo_num,mo_num,N_states,N_states)]
  ...
  BEGIN_DOC
  ! Real one-electron density matrix in the MO basis for the standard (non-CS)
  ! wave function, computed as a state-averaged matrix.
  !
  ! dm(p,q,istate,jstate) = sum_{ij} C_i(istate) * C_j(jstate) * <i| a^+_p a_q |j>
  !
  ! Contributions include:
  !   - diagonal (i==j): orbital occupation of alpha and beta electrons
  !   - off-diagonal single excitations (degree == 1)
  END_DOC
