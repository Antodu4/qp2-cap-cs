subroutine ref_idx_complex_cs(l_det,l_coef,Ndet,N_st,Nint)

  implicit none

  BEGIN_DOC
  ! Prints a table of the dominant determinants in the CS-CIPSI wave function.
  !
  ! For each electronic state, determinants are sorted by decreasing absolute
  ! value of their complex coefficient |C_I|. The top N (up to 6) determinants
  ! are listed, together with their excitation-level relative to the HF reference
  ! and the particle/hole orbital indices.
  !
  ! Input:
  !   l_det(Nint, 2, Ndet)  -- bit-string determinants
  !   l_coef(Ndet, N_st)    -- complex CI coefficients
  !   Ndet                  -- number of determinants
  !   N_st                  -- number of electronic states
  !   Nint                  -- number of integers per spin-determinant
  END_DOC
  ...
  write(*,'(A)') ''
  write(*,'(A)') ' ==========================================================='
  write(*,'(A)') '  List of dominant determinants in CIPSI-CS wave function:  '
  write(*,'(A)') ' ==========================================================='
  write(*,'(A)') ''
