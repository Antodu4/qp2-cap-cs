...
subroutine davidson_diag_hjj_sjj_complex(dets_in,u_in,H_jj,s2_out,energies,dim_in,sze,N_st,N_st_diag_in,Nint,converged)
  ...
  if (do_qr_dav) then
    ...
    ! Orthogonalization using the c-bilinear product (u^T v, no conjg)
    ! Must be consistent with zgemm('T',...) used to build h and s_tmp below.
    ! ortho_qr_complex uses zgeqrf/zungqr (hermitian u†v): if U is c-ortho but
    ! not hermitian-ortho, B = U^T U ≠ I and zggev produces garbage eigenvectors.
